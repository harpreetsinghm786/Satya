...
public class Mask_section extends Fragment {


   RecyclerView mask_rv;
   ...
   DatabaseReference mdatabase4;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
       ...
       View v= inflater.inflate(R.layout.fragment_mask_section, container, false);
       ...
       mask_rv=v.findViewById(R.id.mask_recycler_view);
       progressbar=v.findViewById(R.id.progress);
       progressbar.setVisibility(View.VISIBLE);
        GridLayoutManager gridLayoutManager=new GridLayoutManager(getContext(),2,RecyclerView.VERTICAL,false);

        mask_rv.setLayoutManager(gridLayoutManager);
        list=new ArrayList<>();
        mdatabase4 = FirebaseDatabase.getInstance().getReference("search_list");
        ...
        mdatabase4.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot dataSnapshot1:dataSnapshot.getChildren()){
                    product_detail product=dataSnapshot1.getValue(com.official.pb.satya.product_detail.class);
                    if(product.getTags().toLowerCase().contains("mask")||product.getTags().toLowerCase().contains("masks")){
                        list.add(product);
                    }
                }
                adapter=new genre_detail_items_recycler_view_adapter(getContext(),list);
                mask_rv.setAdapter(adapter);
